  ...
  try {
    ...
    const rows = rawRows
      ...
      .map((row) => ({
        ...
        "รหัส PPSPECIAL": row.ppspecial || "",
        "ชื่อรหัส": row.ppspecial_name || "",
        "สถานะ": errorTypeLabel(row.error_type),
      }));
    const headers = ["หน่วยบริการ", "ชื่อหน่วยบริการ", "สังกัด", "ปีงบประมาณ", "PID", "SEQ", "วันที่รับบริการ", "รหัส PPSPECIAL", "ชื่อรหัส", "สถานะ"];
    const workbook = XLSX.utils.book_new();
    const worksheet = rows.length
      ? XLSX.utils.json_to_sheet(rows, { header: headers })
      : XLSX.utils.aoa_to_sheet([headers]);
    worksheet["!cols"] = headers.map((header) => ({ wch: Math.max(header.length + 2, 14) }));
    XLSX.utils.book_append_sheet(workbook, worksheet, "specialpp_error");
    const buffer = XLSX.write(workbook, { bookType: "xlsx", type: "buffer" });
    const suffix = [fiscalYear, requestedHospcode].join("_");
    return new Response(buffer, {
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="specialpp_error_${suffix}.xlsx"`,
      },
    });
  } catch (error) {
