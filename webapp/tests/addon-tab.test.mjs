...
test("AddonTab appends fixed items (Dashboard-จังหวัด, ดาวน์โหลด) after DB items with a separator", () => {
  ...
  // รายการคงที่ต้องต่อ session-id ด้วย (ค่าจาก API, fallback none)
  assert.match(addonTabSource, /href=\{appendSessionId\(item\.url, sessionId\)\}/);
  assert.match(addonTabSource, /setSessionId\(data\?\.sessionId \|\| "none"\)/);
});

test("addon-url API always appends session-id (cid_hash, or 'none' when not logged in)", () => {
  ...
  // ต่อ session-id เสมอ + fallback "none" + คืน sessionId ให้ client
  assert.match(routeSource, /const sessionId = cidHash \|\| "none"/);
  assert.match(routeSource, /session-id=\$\{encodeURIComponent\(sessionId\)\}/);
  assert.match(routeSource, /Response\.json\(\{ items, sessionId \}\)/);
});
