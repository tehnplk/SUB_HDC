...
function getAesKey(env) {
  const raw = env.SECRET_KEY || "change_me";
...
async function importFile(connection, file, batchSize, onDuplicate, onBatchComplete) {
  ...
  const importColumns = file.columns
    .filter((column) => column.toLowerCase() !== "import_date_time")
    .filter((column) => existingColumns.has(column.toLowerCase()))
    .map((column) => existingColumns.get(column.toLowerCase()));
  const missingColumns = file.columns.filter((column) => !existingColumns.has(column.toLowerCase()));

  if (existingColumns.has("file_name")) {
    importColumns.push(existingColumns.get("file_name"));
  }
  ...
  for (let start = 0; start < file.rows.length; start += batchSize) {
    ...
    for (const row of batch) {
      ...
      for (const column of importColumns) {
        values.push(column.toLowerCase() === "file_name" ? file.fileName : rowByColumn.get(column.toLowerCase()));
      }
    }
...
async function truncateImportTables(pool, files) {
  const tableNames = [...new Set(files.map((file) => file.tableName))];
  const conn = await pool.getConnection();
  try {
    await conn.query("SET FOREIGN_KEY_CHECKS = 0");
    for (const tableName of tableNames) {
      await conn.query(`TRUNCATE TABLE ${quoteIdentifier(tableName)}`);
    }
  } finally {
...
function parseArgs() {
  ...
  for (let index = 2; index < process.argv.length; index += 1) {
    ...
    else if (arg === "--truncate-first") args.truncateFirst = true;
    else if (arg === "--progress") args.progress = true;
    else throw new Error(`Unknown argument: ${arg}`);
    if (!["--progress", "--truncate-first"].includes(arg)) index += 1;
  }
...
async function main() {
  ...
  try {
    if (args.truncateFirst) {
      await truncateImportTables(pool, files);
    }
